...
class Player:
  def get_api_facade(self, host, port):
    return foosballapi.facade.NflPlayerFacade(host, port, self)
...
class OffencePlayer(Player):
  ...
  def __add_stat_to_dictionary(self, nfl_game_key, foosball_key, stats_dict):
    if nfl_game_key in self.player.__dict__:
      stats_dict[foosball_key] = self.player.__dict__[nfl_game_key]
